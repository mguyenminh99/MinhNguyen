...
//                    $url_redirect = $_SERVER['SCRIPT_NAME'] . '/gio-hang-cua-ban';
            ...
            // lặp các phàn từ tron giỏ hàng và gán lại số lượng tương ứng cho từng phần tử theo id của sản phẩm
            foreach ($_SESSION['cart'] as $product_id => $cart){
...
//        lên
        ...
        // + Tạo 1 mảng để chứa các thông tin cần th    iết của
        //giỏ hàng là name, price, avatar
        $sale = $product['sale']/100 * $product['price'];
        $last_price = $product['price'] - $sale;
        $cart = [
            'name' => $product['title'],
            'price' => $last_price,
...
//        Thêm vào giỏ -> thêm mới 1 giỏ hàng
    ...
    public function delete(){
        // nếu trong rewrite đã có validate bằng regex, ví dụ phải là số, thì bên PHP không cần validate lại nữa
//        echo "<pre>";
//        print_r($_GET);
//        echo "</pre>";
        $prodục_id = $_GET['id'];
        // xóa sản phẩm trong giỏ hàng dựa theo id bắt được
        unset($_SESSION['cart'][$prodục_id]);
        // kiểm tra nế xóa hết toàn bộ sản phẩm trong giỏ hàng rồi thì xóa luôn giỏ hàng đi
